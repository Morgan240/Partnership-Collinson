...
const inputStyle = { backgroundColor: "rgba(255,255,255,0.08)", borderColor: "rgba(255,255,255,0.2)" };
const inputClass = "w-full h-14 px-5 text-base rounded-xl border-2 text-white placeholder-white/40 outline-none focus:border-white/40 transition-colors";
...
const CTASection = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      emailjs
        .send(SERVICE_ID, TEMPLATE_ID, { name, email, phone: cleanPhone, time }, PUBLIC_KEY)
        .catch(() => {});
